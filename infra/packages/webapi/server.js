...
// replace the entire app.post handler with the following code -------- for rag memory
app.post("/chat", async (req, res) => {
  ...
  try {
    // Build final messages array as my response to the user
    const messages = [
      systemMessage,
    ...
    console.error(err);
    res.status(500).json({
      error: "Model call failed due to some issues",
      message: err.message,
